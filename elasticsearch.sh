...
if [ "$1" = "start" ] ; then
    ...
    docker start mc-kibana &>/dev/null || docker run --name=mc-kibana -e "ELASTICSEARCH_URL=http://$DOCKER_MACHINE_IP:9200" -p 5601:5601 -d kibana &>/dev/null && docker exec -it mc-kibana kibana plugin --install elastic/sense &>/dev/null && docker stop mc-kibana &>/dev/null && docker start mc-kibana &>/dev/null
    echo
