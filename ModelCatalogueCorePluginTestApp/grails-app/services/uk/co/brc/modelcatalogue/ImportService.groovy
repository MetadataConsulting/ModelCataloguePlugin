...
class ImportService {
	...
	private static functions = [
		...
		{ tokens ->
            ...
            ext.put("A", tokens[9].take(255));
            ext.put("B", tokens[10].take(255));
            ext.put("C", tokens[11].take(255));
            ext.put("D", tokens[12].take(255));
            ext.put("E", tokens[13].take(255));
            ext.put("F",tokens[14].take(255));
            ext.put("G", tokens[15].take(255));
            ext.put("H", tokens[16]);
	...
	private static importModels(categories, ConceptualDomain conceptualDomain)
	{
		...
		categories.inject{ parentName, childName ->
			...
			if(childName.equals("")) { return parentName; }
            ...
            if(namedChildren){ matches = namedChildren.childOf.contains(parentName)}
	...
	private static importDataTypes(name, dataType){
        ...
        if(!dataTypeReturn){ dataTypeReturn = new DataType(name: "String").save()}
