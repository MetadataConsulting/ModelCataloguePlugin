...
# reload in draft mode
  ...
  actionsProvider.registerActionInRole 'generate-merge-models', actionsProvider.ROLE_LIST_ACTION, ['$scope', 'security',
    ...
    ($scope, security, catalogue, modelCatalogueApiRoot, enhance, rest, messages, $state)->
      ...
      {
        position: 100
        label: 'Suggest New Relations'
        icon: 'fa fa-flash'
        type: 'primary'
        action: ->
          messages.prompt('Suggest New Relations', "Suggestions to optimalize catalogue will be generated. This may take a long time depending on complexity of the catalogue. You can rerun the action later to clean all resolved batches generated by this action.", {type: 'generate-suggestions'}).then ->
