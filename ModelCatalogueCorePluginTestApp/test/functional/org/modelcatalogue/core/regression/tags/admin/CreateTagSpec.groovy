...
class CreateTagSpec extends AbstractModelCatalogueGebSpec {
    ...
    def "Navigate to create a tag page"() {
        ...
        click Common.create
        then:
        check Common.modalHeader is "Create Data Element"
    }

    def "create a tag"() {
        when:
        fill Common.nameLabel with("TESTING_TAG")
        fill Common.modelCatalogueId with "M-234"
        fill Common.description with "I am creating a tag ${System.currentTimeMillis()}"
        click Common.save
