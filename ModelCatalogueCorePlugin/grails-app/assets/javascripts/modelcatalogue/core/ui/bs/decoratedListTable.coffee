angular.module('mc.core.ui.bs.decoratedListTable', ['mc.core.ui.decoratedList', 'ngSanitize']).run [ '$templateCache', ($templateCache) ->
    ...
    <div>
      <table ng-show="list.total>0" class="dl-table table">
        ...
        <tbody>
             <tr class="dl-table-item-row" ng-repeat="element in list.list" ng-switch="hasSelection()" ng-class="{'warning': element.status == 'DRAFT' || element.relation.status == 'DRAFT', 'info': element.status == 'PENDING' || element.relation.status == 'PENDING', 'danger': element.status == 'DEPRECATED' || element.relation.status == 'DEPRECATED'}">
