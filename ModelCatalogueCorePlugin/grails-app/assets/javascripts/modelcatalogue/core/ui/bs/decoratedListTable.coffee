        ...
        <tbody>
             <tr class="dl-table-item-row" ng-repeat="element in list.list" ng-switch="hasSelection()">
              ...
              <td class="dl-table-actions-item-cell" ng-if="actions">
                <a ng-repeat="action in actions" class="btn" ng-class="getActionClass(action)" ng-click="performAction(action.action, element, list)"><span ng-if="action.icon" class="glyphicon" ng-class="'glyphicon-' + action.icon"></span><span ng-if="action.icon &amp;&amp; action.title">&nbsp;&nbsp;</span>{{action.title}}</a>
              </td>
            </tr>
        </tbody>
        <tfoot>
