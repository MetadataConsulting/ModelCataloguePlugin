    ...
    $templateCache.put 'modelcatalogue/core/ui/elementsAsTags.html', '''
      <div class="tags">
        <span ng-repeat="element in elements">
            ...
            <span ng-if="!(isString(element) || element.create)" class="label label-primary" title="This is an existing item"><span class="with-pointer" ng-click="openElementInNewWindow(element)">{{element.name}}</span> <a ng-click="removeItem($index)" id="remove-tag-{{$index}}" class="remove-tag"><span class="glyphicon glyphicon-remove"></span></a></span>
        </span>
      </div>
    '''
  ]