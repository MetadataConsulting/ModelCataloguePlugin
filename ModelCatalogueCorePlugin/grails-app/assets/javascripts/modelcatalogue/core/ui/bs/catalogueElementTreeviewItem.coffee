    ...
    <li class="catalogue-element-treeview-item">
      <div class="catalogue-element-treeview-text-content" ng-class="{'active': element.$$active, 'archived': element.$$archived}">
        ...
        <span class="catalogue-element-treeview-labels">
          ...
          <span class="catalogue-element-treeview-name" ng-class="{'text-warning': element.status == 'DRAFT', 'text-info': element.status == 'PENDING', 'text-danger': (element.status == 'DEPRECATED' || element.undone)}" ng-click="select(element)"><span ng-class="element.getIcon()" class="text-muted"></span> {{element.$$localName || element.name}}<small class="text-muted" ng-if="element.$$localName"> {{element.name}}</small><small class="text-muted"> <span ng-if="element.latestVersionId" class="catalogue-element-treeview-version-number">{{element.getDataModelWithVersion()}}</span></small></span>
          <small class="text-muted" ng-if="element.$$href"> <a class="catalogue-element-treeview-link" ng-href="{{element.$$href}}" title="{{element.$$localName || element.name}}" target="_blank"><span class="fa fa-external-link text-muted"></span></a></small>
        </span>
      </div>
      <ul ng-if="element.$$children" ng-hide="element.$$collapsed" class="catalogue-element-treeview-list">
        ...
        <li ng-if="element.$$numberOfChildren > element.$$children.length" class="catalogue-element-treeview-item">
          <span class="catalogue-element-treeview-labels" ng-click="element.$$showMore()">
            <a class="catalogue-element-treeview-icon btn btn-link catalogue-element-treeview-show-more" ng-class="'show-more-' + nodeid"><span class="fa fa-fw fa-chevron-down"></span></a> <a class="text-muted">Show more</a>
          </span>
        </li>
      </ul>
    </li>
