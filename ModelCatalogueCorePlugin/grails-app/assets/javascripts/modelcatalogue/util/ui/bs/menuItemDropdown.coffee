angular.module('mc.util.ui.bs.menuItemDropdown', ['mc.util.ui.menuItemDropdown', 'mc.util.ui.menuItemSingle']).run [ '$templateCache', ($templateCache) ->
  $templateCache.put 'modelcatalogue/util/ui/menuItemDropdown.html', '''
      <li id="{{action.id}}-menu-item" class="dropdown" ng-class="{'active': action.active}" ng-show="action.children.length > 0">
            <a id="{{action.id}}-menu-item-link" href="#" class="dropdown-toggle" data-toggle="dropdown"><span ng-show="action.icon" ng-class="action.icon"></span><span ng-show="action.icon">&nbsp;</span>{{action.label}} <b class="caret"></b></a>
