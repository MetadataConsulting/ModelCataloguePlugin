angular.module('mc.util.ui.bs.actionButtonDropdown', ['mc.util.ui.actionButtonDropdown']).run [ '$templateCache', ($templateCache) ->
  $templateCache.put 'modelcatalogue/util/ui/actionButtonDropdown.html', '''
      <span class="btn-group" dropdown>
          <button id="{{action.id}}" ng-if="!action.abstract" ng-disabled="action.disabled || action.children.length == 0" title="{{action.label}}" type="button" class="btn" ng-class="'btn-' + (noColors ? 'default' : action.type) + ' btn-' + size" ng-click="action.run()"><span ng-show="action.icon" class="glyphicon" ng-class="'glyphicon-' + action.icon"></span><span ng-hide="iconOnly"> {{action.label}}</span></button>
          <button id="{{action.id}}" ng-if="!action.abstract" ng-disabled="action.disabled || action.children.length == 0" title="{{action.label}}" type="button" class="btn dropdown-toggle" ng-class="'btn-' + (noColors ? 'default' : action.type) + ' btn-' + size">
            <span class="caret"></span>
            <span class="sr-only">Split button!</span>
          </button>
          <button id="{{action.id}}" ng-if=" action.abstract" ng-disabled="action.disabled || action.children.length == 0" type="button" class="btn dropdown-toggle" ng-class="'btn-' + (noColors ? 'default' : action.type) + ' btn-' + size">
