angular.module('mc.util.ui.bs.actionButtonDropdown', []).run [ '$templateCache', ($templateCache) ->
  $templateCache.put 'modelcatalogue/util/ui/actionButtonDropdown.html', '''
      <span class="btn-group" dropdown dropdown-append-to-body ng-class="::({'pull-right': action.expandToLeft})">
          ...
          <button id="{{::action.id}}BtnDropdown" ng-if="::!action.abstract" ng-disabled="::(action.disabled || action.children.length == 0)" title="{{::action.label}}" type="button" dropdown-toggle class="btn dropdown-toggle" ng-class="::('btn-' + (noColors ? 'default' : action.type) + ' btn-' + size)">
            <span class="caret"></span>
            <span class="sr-only">Split button!</span>
          </button>
          <button id="{{::action.id}}Btn" ng-if="::action.abstract" ng-disabled="::(action.disabled || action.children.length == 0)" type="button" class="btn dropdown-toggle" dropdown-toggle ng-class="::('btn-' + (noColors ? 'default' : action.type) + ' btn-' + size)">
