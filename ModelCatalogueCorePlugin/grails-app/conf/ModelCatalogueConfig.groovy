...
modelcatalogue.defaults.relationshiptypes =  [
        ...
        [name: "definition", sourceToDestination: "defines", destinationToSource: "defined within", sourceClass: DataModel, destinationClass: CatalogueElement, versionSpecific: true, sourceToDestinationDescription: "Data models can define multiple elements. Based on this relationship you can narrow the elements shown in the catalogue using the data model filter in the bottom left corner. When data model is finalized all defined elements are finalized as well.", destinationToSourceDescription: "Any element can be defined within multiple data models. When new draft of the element is created then drafts for data models are created as well."],
