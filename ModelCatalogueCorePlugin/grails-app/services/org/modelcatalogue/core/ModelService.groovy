...
class ModelService {
	...
	def printXSDModel(Model targetModel) {
			...
			'xs:complexType'(name:'metadata'){
				'xs:sequence'{
					'xs:element'(name:'schema-name',minOccurs:'1',maxOccurs:'1'){
						'xs:simpleType'{
							'xs:restriction'(base:'xs:string'){
								'xs:enumeration'(value:targetModel.ext.get(XSD_SCHEMA_NAME))
							}
						}
					}
					'xs:element'(name:'schema-version',minOccurs:'1',maxOccurs:'1'){
						'xs:simpleType'{
							'xs:restriction'(base:'xs:string'){
								'xs:enumeration'(value:targetModel.ext.get(XSD_SCHEMA_VERSION))
							}
						}
					}
		...
		println('<?xml version="1.0" encoding="UTF-8"?>' + writer.toString())
