...
abstract class AbstractIntegrationSpec extends IntegrationSpec {
    ...
    def loadFixtures(){
        if(CatalogueElement.count()==0){
            fixtures = fixtureLoader.load("assets/*", "batches/*", "dataTypes/*", "enumeratedTypes/*", "measurementUnits/*", "conceptualDomains/*", "models/*", "relationshipTypes/*").load("actions/*", "valueDomains/*").load("dataElements/*").load("extensions/*")
        }
