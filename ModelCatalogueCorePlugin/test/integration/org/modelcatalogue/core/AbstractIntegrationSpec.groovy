...
abstract class AbstractIntegrationSpec extends IntegrationSpec {
    ...
    def loadFixtures(){
        if(CatalogueElement.count()==0){
            fixtures = fixtureLoader.load("assets/*", "batches/*", "dataTypes/*", "enumeratedTypes/*", "measurementUnits/*", "dataElements/*", "conceptualDomains/*", "models/*", "relationshipTypes/*", "classifications/*").load("actions/*", "valueDomains/*", "extensions/*")
        }
