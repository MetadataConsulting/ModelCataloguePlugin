...
abstract class AbstractIntegrationSpec extends IntegrationSpec {
    ...
    def loadFixtures(){
        if(CatalogueElement.count()==0){
            fixtures = fixtureLoader.load("assets/*", "batch/*", "dataTypes/*", "enumeratedTypes/*", "measurementUnits/*", "dataElements/*", "conceptualDomains/*", "models/*", "relationshipTypes/*").load("valueDomains/*", "extensions/*")
        }
