...
@Log4j
class SimpleDraftChain extends PublishingChain {

    private final DraftContext context

    private SimpleDraftChain(DataModel published, DraftContext context) {
        super(published)
    ...
    static SimpleDraftChain create(DataModel published, DraftContext strategy) {
        return new SimpleDraftChain(published, strategy)
    }
    ...
    private <T extends CatalogueElement> T createDraft(T element, DataModel draftDataModel, Publisher<CatalogueElement> archiver) {
        ...
        log.debug("... cretated draft for $element ($context)")
