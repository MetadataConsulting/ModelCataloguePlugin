...
@CompileStatic class UniqueChecker implements ConventionChecker {
    ...
    @Override
    def <T extends CatalogueElement & GroovyObject> void check(DataModel model, Class<T> resource, T item, String property, String ignored, String messageOverride) {
        ...
        if (criteria.count() > 1) {
            model.errors.reject('uniqueChecker.duplicate', [model, ignored, item, property, ignored] as Object[], messageOverride ?: "Property {3} must be unique for every {2} within {0}")
        }
    }
}
