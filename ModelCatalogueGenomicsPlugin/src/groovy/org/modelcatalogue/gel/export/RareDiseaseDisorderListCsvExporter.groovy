    ...
    def descendModels(DataClass model, lines, level, groupDescriptions) {
        ...
        switch (level) {
            ...
            case 2:     // add disease group id,description
                String groupDescription = "${model.ext.get('http://www.modelcatalogue.org/metadata/genomics/#gel-id')?:model.getCombinedVersion()},${modelName}"
                groupDescriptions.put(level, groupDescription)
                break

            case 3: // add disease sub-group id,description
                String groupDescription = "${groupDescriptions.get(level - 1)},${model.ext.get('http://www.modelcatalogue.org/metadata/genomics/#gel-id')?:model.getCombinedVersion()},${modelName}"
                groupDescriptions.put(level, groupDescription)
                break

            case 4: // generate line and add to list
                line << groupDescriptions.get(level - 1)
                line << "${model.ext.get('http://www.modelcatalogue.org/metadata/genomics/#gel-id')?:model.getCombinedVersion()}"
                line << modelName
                lines << line.join(',')
